...
import { topicMastery } from '../data/topicMastery';
import { useStudyProgress } from '../hooks/useStudyProgress';

const baseSections = [['exam-scope', 'Что на экзамене'], ['intuition', 'Интуиция'], ['definition', 'Определения'], ['ideas', 'Главные идеи'], ['visualization', 'Визуализация'], ['example', 'Пример'], ['mistakes', 'Ошибки'], ['practice', 'Практика']];
...
export default function TopicPage() {
  ...
  const mastery = topicMastery[slug];
  ...
  return (
    <main className="lesson-page">
      ...
      <article className="lesson-article">
        ...
        <section id="mistakes" className="lesson-section reveal"><p className="section-kicker">07 · Типичные ошибки</p><h2>Где чаще всего теряются баллы</h2><div className="mistake-list">{lesson.mistakes.map((text, index) => <article key={text}><span>{String(index + 1).padStart(2, '0')}</span><div><h3>{text.split(':')[0]}</h3><p><GlossaryText>{text}</GlossaryText></p></div></article>)}</div><DoneButton done={completed.has('mistakes')} onClick={() => toggle('mistakes')} /></section>

        <section id="practice" className="lesson-section reveal"><p className="section-kicker">08 · Первая самопроверка</p><h2>Три задания после главы</h2><ol className="practice-list">{lesson.practice.map((task) => <li key={task}><GlossaryText>{task}</GlossaryText></li>)}</ol><p>Решайте в таком порядке: назвать тип → выписать теорему или алгоритм → выполнить вычисления → проверить ответ. Если не можете назвать тип за 30 секунд, вернитесь к блоку «Главные идеи».</p><DoneButton done={completed.has('practice')} onClick={() => toggle('practice')} /></section>
        <DeepDive content={deepDive} />
        <TopicMastery topicId={slug} content={mastery} done={completed.has('mastery')} onDone={() => toggle('mastery')} />
        <button className={`complete-topic ${completed.has('complete') ? 'completed' : ''}`} onClick={() => toggle('complete')}>{completed.has('complete') ? 'Тема отмечена завершённой' : 'Завершить тему'}</button>
      </article>
